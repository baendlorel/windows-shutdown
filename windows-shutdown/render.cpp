...
void DrawToMemoryDC(HDC hdcMem, int w, int h, BYTE alpha) {
  ...
  if (appState.isCountingDown) {
    ...
    DrawBeautifulText(graphics, fullText.c_str(), font, (REAL)w, y,
                      Color(255, 255, 255, 255), Color(128, 0, 0, 0));
    ...
    RectF cancelBounds;
    graphics.MeasureString(cancelText.c_str(), -1, &smallFont, layoutRect, &format, &cancelBounds);
    ...
    FontFamily fontFamily(L"Arial");
    Gdiplus::Font instructionFont(&fontFamily,
    ...
    DrawBeautifulText(graphics, instructionText.c_str(), instructionFont,
                      (REAL)w, textY, Color(255, 200, 200, 200),
                      Color(80, 0, 0, 0));
  }
}

void DrawShadow(Graphics& graphics, const wchar_t* text,
                const Gdiplus::Font& font, REAL width, REAL y,
                const Color& color) {
  StringFormat format;
  format.SetAlignment(StringAlignmentCenter);
